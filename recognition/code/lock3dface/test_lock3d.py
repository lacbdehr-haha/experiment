# coding=utf-8
from lock3d_dataloader import lock3d_testloader
import torch
import torchvision.transforms as transforms
import numpy as np
from net import Led3Dnet_soft, con_layer
import argparse
...
def eval_recog(probe_data, gallery_data):
    ...
    gallery_ids = list(gallery_ids)     # gallery_ids:以列表的形式存放
...
def feature_extract(root_dir, model, protocl_file, flow, cond_layer):
    backbone.eval()     # backbone=Led3Dnet_soft()
    ...
    dataset = lock3d_testloader(root_dir, protocl_file, TFS)
    ...
    return feature_dict     # 描述特征的字典


def TestA(root_dir, backbone, gallery_file, nu_file, fe_file, oc_file, ps_file, tm_file, flow, cond_layer):
    ...
    oc_data = feature_extract(root_dir, backbone, oc_file, flow, cond_layer)
    print("finish calc oc_data")
    ps_data = feature_extract(root_dir, backbone, ps_file, flow, cond_layer)
    print("finish calc ps_data")
    tm_data = feature_extract(root_dir, backbone, tm_file, flow, cond_layer)
    print("finish calc tm_data")
    nu_acc, nu_top1, nu_total = eval_recog(nu_data, gallery_data)
    fe_acc, fe_top1, fe_total = eval_recog(fe_data, gallery_data)
    oc_acc, oc_top1, oc_total = eval_recog(oc_data, gallery_data)
    ps_acc, ps_top1, ps_total = eval_recog(ps_data, gallery_data)
    tm_acc, tm_top1, tm_total = eval_recog(tm_data, gallery_data)
    avg_acc = (nu_top1+fe_top1+oc_top1+ps_top1+tm_top1)/(nu_total+fe_total+oc_total+ps_total+tm_total)
    logger.info('DatasetA_acc_nu:{}, fe:{}, oc:{}, ps:{}, tm:{}, avg:{}'.format(nu_acc, fe_acc, oc_acc, ps_acc, tm_acc, avg_acc))
...
parser.add_argument('--num_class', default=340, type=int,
...
if __name__ == '__main__':
    list_dir = '../data/lock3d/protocol/file/'

    device = torch.device("cuda:0")
    gallery_A_file = list_dir + 'gallery_D.txt'
    nu_file = list_dir + 'probe_nu.txt'
    fe_file = list_dir + 'probe_fe.txt'
    oc_file = list_dir + 'probe_oc.txt'
    ps_file = list_dir + 'probe_ps.txt'
    tm_file = list_dir + 'probe_tm.txt'
    ...
    for m in range(1, 51, 2):
        ...
        root_dir = '../data/lock3d/protocol/train/'
        TestA(root_dir, backbone, gallery_A_file, nu_file, fe_file, oc_file, ps_file, tm_file, flow, cond_layer)